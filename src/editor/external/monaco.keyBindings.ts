...
export function addKeyBindings(editor: EditorType, monaco: MonacoType) {
  editor.addAction({
    ...
    run: (ed: EditorType) => {
      ...
      const isTogglingCommentingOn = values.slice(startLineNumber - 1, endLineNumber).some((v: string) => !(v === '') && !isCommented(v));

      const updatedValues = values.map((v: string, i: number) =>
        i >= startLineNumber - 1 && i <= endLineNumber - 1 ? toggleCommenting(v, isTogglingCommentingOn) : v
      );
      ...
      ed.setSelection({
        ...selection,
        selectionStartColumn: isTogglingCommentingOn ? selectionStartColumn + 3 : selectionStartColumn - 3,
        positionColumn: isTogglingCommentingOn ? positionColumn + 3 : positionColumn - 3,
      });
